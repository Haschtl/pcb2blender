...
import re
import tempfile, random, shutil, struct, math, io
...
from .shared import PCB, COMPONENTS, LAYERS, LAYERS_BOUNDS, BOARDS, BOUNDS, STACKED, PADS, INCLUDED_LAYERS, REQUIRED_MEMBERS, SKIA_MAGIC, INCH_TO_MM, regex_filter_components, PCB_THICKNESS, MM_TO_M, PCB2_LAYER_NAMES, FIX_X3D_SCALE, M_TO_MM
...
class PCB2BLENDER_OT_import_pcb3d(bpy.types.Operator, ImportHelper):
    ...
    pcb_material:      EnumProperty(name="PCB Material", default="RASTERIZED",
        items=(("RASTERIZED", "Rasterized (Cycles)", "PERFORMANCE", ""), ("3D", "3D (deprecated)", "")))
    ...
    def _execute(self, context, pcb):
        ...
        if has_svg2blender() and self.import_fpnl and self.fpnl_path != "":
            if Path(self.fpnl_path).is_file():
                ...
                self.warning(
                    f"frontpanel file \"{self.fpnl_path}\" does not exist")
...
def menu_func_import_pcb3d(self, context):
    self.layout.operator(
        PCB2BLENDER_OT_import_pcb3d.bl_idname, text="PCB (.pcb3d)")
